...
(function(){
  ...
  function openAuth(){if(!window.bmsDb)return toast(config.supabaseUrl&&publicKey?'Connexion à la base en cours, patientez un instant puis réessayez.':'Configurez Supabase avant d’ouvrir l’espace équipe.');authModal.classList.remove('hidden');document.querySelector('#authEmail').focus()}
  function closeAuth(){authModal.classList.add('hidden');authMessage.textContent=''}
  async function openBackofficeIfAllowed(){if(!window.bmsDb)return openAuth();const {data:{user}}=await window.bmsDb.auth.getUser();if(!user)return openAuth();const {data:profile}=await window.bmsDb.from('profiles').select('role').eq('id',user.id).single();if(!profile||!staffRoles.includes(profile.role)){await window.bmsDb.auth.signOut();return toast('Ce compte n’a pas accès à l’espace équipe.')}showView('backoffice');if(window.bmsRefreshDashboard)window.bmsRefreshDashboard()}
